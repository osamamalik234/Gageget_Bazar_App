...
class StripePayment2 {
  ...
  Future<void> makePayment(String amount, BuildContext context , ProductModel productModel) async {
    try {
      print(amount);
      ...
      displayPaymentSheet(context, productModel);
    } catch (err) {
      print(err);
    }
  }

  displayPaymentSheet(BuildContext context , ProductModel productModel) async {
    final myProvider = Provider.of<MyProvider>(context, listen: false);
    try {
      await Stripe.instance.presentPaymentSheet().then((value) {
        myProvider.getUserBuyProduct(productModel);
